		...
		if(!TextUtils.isEmpty(imagePath) && new File(imagePath).exists()){
			...
			new Thread(){
				public void run() {
					...
					String dir = getCachePath(activity, "images");
					File file = new File(dir, "picUrl");
					if (file.exists()) {
						file.delete();
					}
					...
					try {
						network.download(imageUrl, file);
					} catch (Throwable e) {
						e.printStackTrace();
						file = null;
					}
					...
					if (file != null && file.exists()) {
						...
						try {
							shareImage = true;
							image = cn.sharesdk.framework.utils.R.getBitmap(path);
						} catch(Throwable t) {
							System.gc();
							try {
								image = cn.sharesdk.framework.utils.R.getBitmap(path, 2);
							} catch(Throwable t1) {
								t1.printStackTrace();
								shareImage = false;
							}
